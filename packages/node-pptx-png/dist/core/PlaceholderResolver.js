import { getXmlAttr, getXmlChild, getXmlChildren } from './PptxParser.js';
 ...
 */
export function getMasterStyleBucket(type) {
    switch (type) {
        case 'title':
        case 'ctrTitle':
            return 'title';
...
export class PlaceholderResolver {
    ...
    resolvePlaceholder(placeholderRef, _slideNode, layoutNode, masterNode) {
        ...
        return {
            ...
            inheritedShapeProps: layoutPh ? getXmlChild(layoutPh, 'p:spPr') : undefined,
            inheritedTextBody: layoutPh ? getXmlChild(layoutPh, 'p:txBody') : undefined,
    ...
    resolveTextStyleChain(placeholderRef, layoutNode, masterNode, defaultTextStyle) {
        ...
        const masterTxBody = masterPh ? getXmlChild(masterPh, 'p:txBody') : undefined;
        const masterLstStyle = masterTxBody ? getXmlChild(masterTxBody, 'a:lstStyle') : undefined;
        if (masterLstStyle) {
            chain.push(masterLstStyle);
        }
        const layoutTxBody = layoutPh ? getXmlChild(layoutPh, 'p:txBody') : undefined;
        const layoutLstStyle = layoutTxBody ? getXmlChild(layoutTxBody, 'a:lstStyle') : undefined;
        if (layoutLstStyle) {
    ...
    findPlaceholderShape(containerNode, type, idx) {
        const cSld = getXmlChild(containerNode, 'p:cSld');
        const spTree = cSld ? getXmlChild(cSld, 'p:spTree') : undefined;
        if (!spTree)
            return undefined;
        const shapes = getXmlChildren(spTree, 'p:sp');
        const candidates = [];
        for (const shape of shapes) {
            const nvSpPr = getXmlChild(shape, 'p:nvSpPr');
            const nvPr = nvSpPr ? getXmlChild(nvSpPr, 'p:nvPr') : undefined;
            const ph = getPresentChild(nvPr, 'p:ph');
            if (!ph)
                continue;
            const phIdxAttr = getXmlAttr(ph, 'idx');
            candidates.push({
                shape,
                phType: getXmlAttr(ph, 'type'),
    ...
    getDefaultTextStyle(type, masterNode) {
        if (!masterNode)
            return undefined;
        const txStyles = getXmlChild(masterNode, 'p:txStyles');
        if (!txStyles)
            return undefined;
        switch (getMasterStyleBucket(type)) {
            case 'title':
                return getXmlChild(txStyles, 'p:titleStyle');
            case 'other':
                return getXmlChild(txStyles, 'p:otherStyle');
            default:
                return getXmlChild(txStyles, 'p:bodyStyle');
        }
    }
    /**
    ...
    getShapePlaceholderType(shapeNode) {
        ...
        const nvPr = nvSpPr ? getXmlChild(nvSpPr, 'p:nvPr') : undefined;
        const ph = getPresentChild(nvPr, 'p:ph');
        return ph ? getXmlAttr(ph, 'type') : undefined;
    }
}
