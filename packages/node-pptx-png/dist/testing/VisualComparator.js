 ...
 */
export async function compareImages(baseline, rendered, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    // Load images (narrow the union: skia-canvas v3 declares separate
    // loadImage overloads for paths and buffers)
    const baselineImg = typeof baseline === 'string' ? await loadImage(baseline) : await loadImage(baseline);
    const renderedImg = typeof rendered === 'string' ? await loadImage(rendered) : await loadImage(rendered);
...
//# sourceMappingURL=VisualComparator.js.map