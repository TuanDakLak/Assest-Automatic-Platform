...
exports.calculatePathBounds = calculatePathBounds;
exports.applyPathToContext = applyPathToContext;
exports.pathToPath2D = pathToPath2D;
...
class PathBuilder {
     ...
     * @param y End point Y
     */
    cubicBezierTo(cp1x, cp1y, cp2x, cp2y, x, y) {
        if (!this.hasMoved) {
 ...
 */
function applyPathToContext(ctx, path, startNewPath = true) {
    if (startNewPath) {
        ctx.beginPath();
    }
    let currentX = 0;
        ...
        switch (segment.type) {
            ...
            case 'arcTo':
                if (segment.points?.[0] && segment.arc) {
                    ...
                    applyArcToContext(ctx, currentX, currentY, endPoint.x, endPoint.y, segment.arc.rx, segment.arc.ry, segment.arc.xAxisRotation, segment.arc.largeArcFlag, segment.arc.sweepFlag);
                    currentX = endPoint.x;
                    currentY = endPoint.y;
                }
                    ...
                    const endPoint = segment.points[0];
                    applyLegacyArcToContext(ctx, currentX, currentY, endPoint.x, endPoint.y, segment.legacyArc.rx, segment.legacyArc.ry, segment.legacyArc.startAngle, segment.legacyArc.swingAngle);
                    currentX = endPoint.x;
                    currentY = endPoint.y;
                }
                break;
 ...
 * @param sweepFlag Direction of the arc (true = clockwise)
 */
function applyArcToContext(ctx, x1, y1, x2, y2, rx, ry, xAxisRotation, largeArcFlag, sweepFlag) {
 ...
 */
function applyLegacyArcToContext(ctx, x1, y1, x2, y2, rx, ry, startAngle, swingAngle) {
    // Handle degenerate cases
    if (x1 === x2 && y1 === y2) {
        return;
    }
...
function pathToPath2D(path) {
    const path2d = new skia_canvas_1.Path2D();
    for (const segment of path.segments) {
        switch (segment.type) {
            case 'moveTo':
                if (segment.points?.[0]) {
                    path2d.moveTo(segment.points[0].x, segment.points[0].y);
                }
                break;
            case 'lineTo':
                if (segment.points?.[0]) {
                    path2d.lineTo(segment.points[0].x, segment.points[0].y);
                }
                break;
            case 'cubicBezierTo':
                if (segment.points && segment.points.length >= 3) {
            ...
            case 'arcTo':
                ...
                throw new Error('Arc segments are not supported in Path2D conversion. Use applyPathToContext instead.');
            case 'close':
                path2d.closePath();
                break;
        }
