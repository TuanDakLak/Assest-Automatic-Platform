...
exports.parseCustomGeometryDefinition = parseCustomGeometryDefinition;
...
function extractPoints(node, expected) {
    const ptNodes = (0, PptxParser_js_1.getXmlChildren)(node, 'a:pt');
    if (ptNodes.length < expected)
        return undefined;
    const values = [];
    for (let i = 0; i < expected; i++) {
        const pt = ptNodes[i];
        const x = toGeomValue((0, PptxParser_js_1.getXmlAttr)(pt, 'x'));
        const y = toGeomValue((0, PptxParser_js_1.getXmlAttr)(pt, 'y'));
...
function parseSegment(tagName, node) {
    switch (tagName) {
        ...
        case 'a:arcTo': {
            const wR = toGeomValue((0, PptxParser_js_1.getXmlAttr)(node, 'wR'));
            const hR = toGeomValue((0, PptxParser_js_1.getXmlAttr)(node, 'hR'));
            const stAng = toGeomValue((0, PptxParser_js_1.getXmlAttr)(node, 'stAng'));
            const swAng = toGeomValue((0, PptxParser_js_1.getXmlAttr)(node, 'swAng'));
...
function parsePathDef(pathNode) {
    const cmds = [];
    const orderedSegments = (0, PptxParser_js_1.getChildrenInDocumentOrder)(pathNode, 'a:path', PATH_SEGMENT_TAGS);
    ...
    if (cmds.length === 0)
        return undefined;
    const pathDef = { cmds };
    const wAttr = (0, PptxParser_js_1.getXmlAttr)(pathNode, 'w');
    const hAttr = (0, PptxParser_js_1.getXmlAttr)(pathNode, 'h');
    if (wAttr !== undefined)
        pathDef.w = parseFloat(wAttr);
    if (hAttr !== undefined)
        pathDef.h = parseFloat(hAttr);
    const fillAttr = (0, PptxParser_js_1.getXmlAttr)(pathNode, 'fill');
    if (fillAttr !== undefined && fillAttr !== 'norm') {
        pathDef.fill = fillAttr;
    }
    const strokeAttr = (0, PptxParser_js_1.getXmlAttr)(pathNode, 'stroke');
 ...
 */
function parseCustomGeometryDefinition(custGeom) {
    const pathLst = (0, PptxParser_js_1.getXmlChild)(custGeom, 'a:pathLst');
    if (!pathLst)
        return undefined;
    const paths = [];
    for (const pathNode of (0, PptxParser_js_1.getXmlChildren)(pathLst, 'a:path')) {
    ...
    const avLst = (0, PptxParser_js_1.getXmlChild)(custGeom, 'a:avLst');
    if (avLst) {
        const av = [];
        for (const gd of (0, PptxParser_js_1.getXmlChildren)(avLst, 'a:gd')) {
    ...
    const gdLst = (0, PptxParser_js_1.getXmlChild)(custGeom, 'a:gdLst');
    if (gdLst) {
        const gd = [];
        for (const gdNode of (0, PptxParser_js_1.getXmlChildren)(gdLst, 'a:gd')) {
            const name = (0, PptxParser_js_1.getXmlAttr)(gdNode, 'name');
            const fmla = (0, PptxParser_js_1.getXmlAttr)(gdNode, 'fmla');
            if (name && fmla) {
                gd.push([name, fmla]);
            }
        }
    ...
    // Text rectangle
    const rectNode = (0, PptxParser_js_1.getXmlChild)(custGeom, 'a:rect');
    if (rectNode) {
        const l = toGeomValue((0, PptxParser_js_1.getXmlAttr)(rectNode, 'l'));
        const t = toGeomValue((0, PptxParser_js_1.getXmlAttr)(rectNode, 't'));
        const r = toGeomValue((0, PptxParser_js_1.getXmlAttr)(rectNode, 'r'));
        const b = toGeomValue((0, PptxParser_js_1.getXmlAttr)(rectNode, 'b'));
