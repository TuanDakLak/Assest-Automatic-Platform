...
exports.StrokeRenderer = exports.DEFAULT_STROKE_WIDTH_EMU = void 0;
exports.getPathEndPoints = getPathEndPoints;
exports.computeLineEndGeometry = computeLineEndGeometry;
exports.shortenPathEnds = shortenPathEnds;
exports.createStrokeRenderer = createStrokeRenderer;
 ...
 */
exports.DEFAULT_STROKE_WIDTH_EMU = 12700;
/**
 ...
 */
function computeLineEndGeometry(point, direction, end, lineWidthPx) {
 ...
 */
function shortenPathEnds(path, headInset, tailInset) {
...
class StrokeRenderer {
    ...
    unitConverter;
    constructor(config) {
        this.logger = config.logger ?? (0, Logger_js_1.createLogger)('warn', 'StrokeRenderer');
    ...
    renderStroke(ctx, path, stroke, scaleX, scaleY) {
        ...
        (0, PathBuilder_js_1.applyPathToContext)(ctx, drawPath, true);
        ctx.stroke();
    ...
    parseStroke(spPr, styleStroke) {
        if (!spPr)
            return styleStroke;
        const ln = (0, PptxParser_js_1.getXmlChild)(spPr, 'a:ln');
        if (!ln)
            return styleStroke;
        // Explicit no fill on the outline beats any style reference.
        // hasXmlChild: a self-closed <a:noFill/> parses to '' which is falsy.
        if ((0, PptxParser_js_1.hasXmlChild)(ln, 'a:noFill')) {
            return undefined;
        }
        // Get stroke width
        const w = (0, PptxParser_js_1.getXmlAttr)(ln, 'w');
        const width = w !== undefined ? parseInt(w, 10) : (styleStroke?.width ?? exports.DEFAULT_STROKE_WIDTH_EMU);
        // Get stroke color
    ...
    resolveThemeLineStyle(style, phClr) {
        ...
        return {
            width: style.width ?? exports.DEFAULT_STROKE_WIDTH_EMU,
    ...
    parseStrokeColor(ln) {
        ...
        const solidFill = (0, PptxParser_js_1.getXmlChild)(ln, 'a:solidFill');
        if (solidFill) {
        ...
        const gradFill = (0, PptxParser_js_1.getXmlChild)(ln, 'a:gradFill');
        if (gradFill) {
            const gsLst = (0, PptxParser_js_1.getXmlChild)(gradFill, 'a:gsLst');
            if (gsLst) {
                const gs = (0, PptxParser_js_1.getXmlChild)(gsLst, 'a:gs');
                if (gs) {
    ...
    parseDashPattern(ln) {
        ...
        if (prstDash) {
            const val = (0, PptxParser_js_1.getXmlAttr)(prstDash, 'val');
            if (val && PRESET_DASH_PATTERNS[val]) {
                return PRESET_DASH_PATTERNS[val];
            }
        }
        // Check for custom dash
        const custDash = (0, PptxParser_js_1.getXmlChild)(ln, 'a:custDash');
        if (custDash) {
    ...
    parseLineEnd(ln, tagName) {
        ...
        if (!(0, PptxParser_js_1.hasXmlChild)(ln, tagName))
            return undefined;
        const lineEnd = (0, PptxParser_js_1.getXmlChild)(ln, tagName);
        const type = lineEnd ? (0, PptxParser_js_1.getXmlAttr)(lineEnd, 'type') : undefined;
        if (!type || !LINE_END_TYPES.has(type))
            return 'none';
        return {
            type: type,
            width: this.parseLineEndSize((0, PptxParser_js_1.getXmlAttr)(lineEnd, 'w')),
            length: this.parseLineEndSize((0, PptxParser_js_1.getXmlAttr)(lineEnd, 'len')),
        };
    }
    /**
    ...
    parseLineJoin(ln) {
        ...
        if ((0, PptxParser_js_1.getXmlChild)(ln, 'a:bevel')) {
            return 'bevel';
        }
        if ((0, PptxParser_js_1.getXmlChild)(ln, 'a:miter')) {
            return 'miter';
        }
        return undefined;
    }
...
exports.StrokeRenderer = StrokeRenderer;
/**
 * Default stroke renderer factory.
 */
function createStrokeRenderer(theme, logger) {
