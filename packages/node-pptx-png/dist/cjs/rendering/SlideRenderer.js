...
const BackgroundRenderer_js_1 = require("./BackgroundRenderer.js");
const ShapeRenderer_js_1 = require("./ShapeRenderer.js");
const GroupShapeRenderer_js_1 = require("./GroupShapeRenderer.js");
const AlternateContentRenderer_js_1 = require("./AlternateContentRenderer.js");
const DiagramRenderer_js_1 = require("./DiagramRenderer.js");
const ChartRenderer_js_1 = require("./ChartRenderer.js");
const ChartParser_js_1 = require("../parsers/ChartParser.js");
const TableRenderer_js_1 = require("./TableRenderer.js");
const TableStyleParser_js_1 = require("../parsers/TableStyleParser.js");
const Logger_js_1 = require("../utils/Logger.js");
const PngOptimizer_js_1 = require("../utils/PngOptimizer.js");
/**
...
function isPlaceholderElement(tagName, node) {
    ...
    if (!wrapperTag)
        return false;
    const nvPr = (0, PptxParser_js_1.getXmlChild)((0, PptxParser_js_1.getXmlChild)(node, wrapperTag), 'p:nvPr');
    return (0, PptxParser_js_1.hasXmlChild)(nvPr, 'p:ph');
}
/**
...
class SlideRenderer {
    ...
    constructor(theme, options = {}, logger) {
        this.logger = logger ?? (0, Logger_js_1.createLogger)('warn', 'SlideRenderer');
        this.theme = theme;
        // mergeRenderOptions ignores explicitly-undefined keys so they cannot
        // clobber defaults (e.g. { width: undefined }).
        this.options = (0, index_js_1.mergeRenderOptions)(options);
        this.backgroundRenderer = new BackgroundRenderer_js_1.BackgroundRenderer(theme, this.logger.child('Background'));
        this.unitConverter = new UnitConverter_js_1.UnitConverter();
        this.chartParser = new ChartParser_js_1.ChartParser({ theme, logger: this.logger.child('ChartParser') });
        this.alternateContentRenderer = new AlternateContentRenderer_js_1.AlternateContentRenderer({
            logger: this.logger.child('AltContent'),
        });
        this.pngOptimizer = new PngOptimizer_js_1.PngOptimizer(this.logger.child('PngOptimizer'));
    }
    /**
    ...
    async renderSlideToContext(canvas, ctx, parser, slideData, slideWidthEmu, slideHeightEmu, layoutNode, masterNode, layoutPath, masterPath, warnings) {
        ...
        const effectiveColorMap = (0, ColorResolver_js_1.resolveEffectiveColorMap)(masterNode, layoutNode, slideData.content);
        let tableStyles = this.tableStylesCache.get(parser);
        if (!tableStyles) {
            tableStyles = await new TableStyleParser_js_1.TableStyleParser({
    ...
    createRenderState(params, sourcePath, rootTag) {
        ...
        return {
            ...
            scaleY,
            warnings,
            shapeRenderer: new ShapeRenderer_js_1.ShapeRenderer({
                theme: this.theme,
                scaleX,
                scaleY,
                parser,
            ...
            groupShapeRenderer: new GroupShapeRenderer_js_1.GroupShapeRenderer({
                scaleX,
                scaleY,
                logger: this.logger.child('GroupShape'),
            }),
            tableRenderer: new TableRenderer_js_1.TableRenderer({
                theme: this.theme,
                colorMap: effectiveColorMap,
                scaleX,
                scaleY,
            ...
            diagramRenderer: new DiagramRenderer_js_1.DiagramRenderer({
                theme: this.theme,
                parser,
                slidePath: sourcePath,
                slideNode: slideData.content,
            ...
            chartRenderer: new ChartRenderer_js_1.ChartRenderer({
    ...
    createCanvas(width, height) {
        const textOptions = this.options.textContrast !== undefined || this.options.textGamma !== undefined
            ...
            : undefined;
        const canvas = textOptions ? new skia_canvas_1.Canvas(width, height, textOptions) : new skia_canvas_1.Canvas(width, height);
    ...
    getBackgroundColor(slideNode, layoutNode, masterNode) {
        return (this.backgroundRenderer.getBackgroundColor(slideNode, layoutNode, masterNode) ?? index_js_1.Colors.white);
    }
    /**
    ...
    async renderShapeTreeOrdered(ctx, state) {
        ...
        const orderedElements = (0, PptxParser_js_1.getOrderedChildren)(spTreeChildren, constants_js_1.SHAPE_ELEMENT_TYPES);
    ...
    async renderGraphicFrame(ctx, graphicFrameNode, state, groupTransform) {
        ...
        const xfrm = (0, PptxParser_js_1.getXmlChild)(graphicFrameNode, 'p:xfrm');
        if (!xfrm) {
        ...
        const x = parseInt((0, PptxParser_js_1.getXmlAttr)(off, 'x') ?? '0', 10);
        const y = parseInt((0, PptxParser_js_1.getXmlAttr)(off, 'y') ?? '0', 10);
        const cx = parseInt((0, PptxParser_js_1.getXmlAttr)(ext, 'cx') ?? '0', 10);
        const cy = parseInt((0, PptxParser_js_1.getXmlAttr)(ext, 'cy') ?? '0', 10);
        ...
        const graphic = (0, PptxParser_js_1.getXmlChild)(graphicFrameNode, 'a:graphic');
        const graphicData = graphic ? (0, PptxParser_js_1.getXmlChild)(graphic, 'a:graphicData') : undefined;
        if (!graphicData) {
            this.logger.debug('GraphicFrame has no graphic data');
            return;
        }
        const uri = (0, PptxParser_js_1.getXmlAttr)(graphicData, 'uri');
        ...
        else if (uri === DiagramRenderer_js_1.DIAGRAM_GRAPHIC_DATA_URI) {
    ...
    findOleSnapshotPic(graphicData) {
        const candidates = [(0, PptxParser_js_1.getXmlChild)(graphicData, 'p:oleObj')];
        const alternate = (0, PptxParser_js_1.getXmlChild)(graphicData, 'mc:AlternateContent');
        if (alternate) {
            ...
            for (const branch of ['mc:Fallback', 'mc:Choice']) {
                const branchNode = (0, PptxParser_js_1.getXmlChild)(alternate, branch);
                if (branchNode)
                    candidates.push((0, PptxParser_js_1.getXmlChild)(branchNode, 'p:oleObj'));
            }
        }
        for (const oleObj of candidates) {
            const pic = oleObj ? (0, PptxParser_js_1.getXmlChild)(oleObj, 'p:pic') : undefined;
            if (pic)
                return pic;
        }
        return undefined;
    ...
    async renderChartFrame(ctx, graphicData, bounds, state) {
        ...
        const chartRef = (0, PptxParser_js_1.getXmlChild)(graphicData, 'c:chart');
        if (!chartRef) {
            this.logger.debug('No chart reference in graphic data');
            return;
        }
        const chartRelId = (0, PptxParser_js_1.getXmlAttr)(chartRef, 'r:id');
        if (!chartRelId) {
    ...
    renderTableFrame(ctx, graphicData, bounds, state) {
        ...
        const tableNode = (0, PptxParser_js_1.getXmlChild)(graphicData, 'a:tbl');
        if (!tableNode) {
