...
class BackgroundRenderer {
    ...
    constructor(theme, logger) {
        this.logger = logger ?? (0, Logger_js_1.createLogger)('warn', 'BackgroundRenderer');
        this.theme = theme;
        this.colorResolver = new ColorResolver_js_1.ColorResolver(theme.colors);
    }
    /**
    ...
    createColorResolverForSlide(slideNode, layoutNode, masterNode) {
        const colorMap = (0, ColorResolver_js_1.resolveEffectiveColorMap)(masterNode, layoutNode, slideNode);
        return new ColorResolver_js_1.ColorResolver(this.theme.colors, colorMap);
    }
    /**
    ...
    renderBackgroundFill(ctx, width, height, background, _supportPicture) {
        if (!background) {
            ...
            this.fillSolid(ctx, width, height, index_js_1.Colors.white);
            return;
        }
        switch (background.type) {
            case 'solid':
                this.fillSolid(ctx, width, height, background.color ?? index_js_1.Colors.white);
                break;
            case 'gradient':
    ...
    parseBackground(node, resolver = this.colorResolver) {
        ...
        const cSld = (0, PptxParser_js_1.getXmlChild)(node, 'p:cSld');
        if (!cSld)
            return undefined;
        const bg = (0, PptxParser_js_1.getXmlChild)(cSld, 'p:bg');
        if (!bg)
            return undefined;
        // Check for background properties (explicit fill)
        const bgPr = (0, PptxParser_js_1.getXmlChild)(bg, 'p:bgPr');
        if (bgPr) {
            return this.parseBgProperties(bgPr, resolver);
        }
        // Check for background reference (theme style)
        const bgRef = (0, PptxParser_js_1.getXmlChild)(bg, 'p:bgRef');
        if (bgRef) {
    ...
    parseBgProperties(bgPr, resolver) {
        ...
        const solidFill = (0, PptxParser_js_1.getXmlChild)(bgPr, 'a:solidFill');
        if (solidFill) {
        ...
        const gradFill = (0, PptxParser_js_1.getXmlChild)(bgPr, 'a:gradFill');
        if (gradFill) {
            return this.parseGradientFill(gradFill, resolver);
        }
        // Check for pattern fill
        const pattFill = (0, PptxParser_js_1.getXmlChild)(bgPr, 'a:pattFill');
        if (pattFill) {
        ...
        const blipFill = (0, PptxParser_js_1.getXmlChild)(bgPr, 'a:blipFill');
        if (blipFill) {
        ...
        // Check for no fill
        const noFill = (0, PptxParser_js_1.getXmlChild)(bgPr, 'a:noFill');
        if (noFill) {
            return { type: 'none' };
        }
        return undefined;
    ...
    parseBgReference(bgRef, resolver) {
        const idx = parseInt((0, PptxParser_js_1.getXmlAttr)(bgRef, 'idx') ?? '0', 10);
    ...
    fillLinearGradient(ctx, width, height, stops, angleDegrees) {
        ...
        const { x0, y0, x1, y1 } = (0, FillRenderer_js_1.computeLinearGradientPoints)(centerX, centerY, diagonal, angleDegrees);
    ...
    getBackgroundColor(slideNode, layoutNode, masterNode) {
        ...
        return index_js_1.Colors.white;
    }
}
