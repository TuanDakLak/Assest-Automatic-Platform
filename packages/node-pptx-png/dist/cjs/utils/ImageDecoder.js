...
const index_js_1 = require("../emf/index.js");
/**
...
class ImageDecoder {
    ...
    warnings;
    constructor(config = {}) {
        this.logger = config.logger ?? (0, Logger_js_1.createLogger)('warn', 'ImageDecoder');
    ...
    detectFormat(buffer) {
        ...
        if ((0, index_js_1.isEmf)(buffer)) {
            return 'emf';
        }
        // Check WMF (placeable 0x9AC6CDD7 key or standard header)
        if ((0, index_js_1.isWmf)(buffer)) {
            return 'wmf';
        }
        return 'unknown';
    }
    ...
    async decode(buffer) {
        ...
        try {
            ...
            const image = await (0, skia_canvas_1.loadImage)(buffer);
            const result = {
                image,
                width: image.width,
                height: image.height,
    ...
    async decodeMetafile(buffer, format) {
        try {
            const parsed = format === 'emf'
                ? new index_js_1.EmfParser({ logger: this.logger.child?.('EmfParser') }).parse(buffer)
                : new index_js_1.WmfParser({ logger: this.logger.child?.('WmfParser') }).parse(buffer);
            const renderer = new index_js_1.EmfRenderer({
                logger: this.logger.child?.('EmfRenderer'),
                warnings: this.warnings,
            });
            const canvas = await renderer.render(parsed);
            const png = await canvas.toBuffer('png');
            const image = await (0, skia_canvas_1.loadImage)(png);
 ...
 */
function createImageDecoder(logger) {
