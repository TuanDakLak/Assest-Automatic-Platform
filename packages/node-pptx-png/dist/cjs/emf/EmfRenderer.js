...
exports.EmfRenderer = void 0;
const skia_canvas_1 = require("skia-canvas");
const Logger_js_1 = require("../utils/Logger.js");
const FontResolver_js_1 = require("../text/FontResolver.js");
...
class EmfRenderer {
    ...
    fontResolver;
    constructor(config = {}) {
        this.logger = config.logger ?? (0, Logger_js_1.createLogger)('warn', 'EmfRenderer');
        this.maxDimensionPx = config.maxDimensionPx ?? 4096;
        this.scale = config.scale ?? 1;
        this.warnings = config.warnings;
        this.fontResolver = new FontResolver_js_1.FontResolver({
    ...
    async render(parsed) {
        ...
        const canvas = new skia_canvas_1.Canvas(outWidth, outHeight);
    ...
    initialState() {
        return {
            mapMode: types_js_1.MAP_MODE.TEXT,
            windowOrg: { x: 0, y: 0 },
            windowExt: { cx: 1, cy: 1 },
            viewportOrg: { x: 0, y: 0 },
            viewportExt: { cx: 1, cy: 1 },
            bkMode: types_js_1.BK_MODE.OPAQUE,
            bkColor: 0x00ffffff,
            textColor: 0x00000000,
            textAlign: 0,
            polyFillMode: types_js_1.POLY_FILL_MODE.ALTERNATE,
            pen: { style: types_js_1.PEN_STYLE.SOLID, width: 0, color: 0x00000000 },
            brush: { style: types_js_1.BRUSH_STYLE.SOLID, color: 0x00ffffff },
            font: { ...DEFAULT_FONT },
            clip: [],
            cur: { x: 0, y: 0 },
        };
    ...
    applyRecord(ctx, state, stack, objects, dibImages, device, record) {
        switch (record.type) {
            ...
            case 'selectClipRgn':
                ...
                if (record.mode === types_js_1.RGN_MODE.COPY) {
                    state.clip = record.rects.length > 0 ? [record.rects.map((r) => ({ ...r }))] : [];
                }
                else if (record.mode === types_js_1.RGN_MODE.AND && record.rects.length > 0) {
    ...
    selectObject(state, objects, index) {
        if ((index & types_js_1.STOCK_OBJECT_FLAG) !== 0) {
            this.selectStockObject(state, index & ~types_js_1.STOCK_OBJECT_FLAG);
            return;
        }
    ...
    selectStockObject(state, id) {
        switch (id) {
            case types_js_1.STOCK_OBJECT.WHITE_BRUSH:
                state.brush = { style: types_js_1.BRUSH_STYLE.SOLID, color: 0x00ffffff };
                break;
            case types_js_1.STOCK_OBJECT.LTGRAY_BRUSH:
                state.brush = { style: types_js_1.BRUSH_STYLE.SOLID, color: 0x00c0c0c0 };
                break;
            case types_js_1.STOCK_OBJECT.GRAY_BRUSH:
                state.brush = { style: types_js_1.BRUSH_STYLE.SOLID, color: 0x00808080 };
                break;
            case types_js_1.STOCK_OBJECT.DKGRAY_BRUSH:
                state.brush = { style: types_js_1.BRUSH_STYLE.SOLID, color: 0x00404040 };
                break;
            case types_js_1.STOCK_OBJECT.BLACK_BRUSH:
                state.brush = { style: types_js_1.BRUSH_STYLE.SOLID, color: 0x00000000 };
                break;
            case types_js_1.STOCK_OBJECT.NULL_BRUSH:
                state.brush = { style: types_js_1.BRUSH_STYLE.NULL, color: 0 };
                break;
            case types_js_1.STOCK_OBJECT.WHITE_PEN:
                state.pen = { style: types_js_1.PEN_STYLE.SOLID, width: 0, color: 0x00ffffff };
                break;
            case types_js_1.STOCK_OBJECT.BLACK_PEN:
            case types_js_1.STOCK_OBJECT.DC_PEN:
                state.pen = { style: types_js_1.PEN_STYLE.SOLID, width: 0, color: 0x00000000 };
                break;
            case types_js_1.STOCK_OBJECT.NULL_PEN:
                state.pen = { style: types_js_1.PEN_STYLE.NULL, width: 0, color: 0 };
                break;
            case types_js_1.STOCK_OBJECT.DC_BRUSH:
                state.brush = { style: types_js_1.BRUSH_STYLE.SOLID, color: 0x00ffffff };
                break;
            case types_js_1.STOCK_OBJECT.OEM_FIXED_FONT:
            case types_js_1.STOCK_OBJECT.ANSI_FIXED_FONT:
            case types_js_1.STOCK_OBJECT.ANSI_VAR_FONT:
            case types_js_1.STOCK_OBJECT.SYSTEM_FONT:
            case types_js_1.STOCK_OBJECT.DEVICE_DEFAULT_FONT:
            case types_js_1.STOCK_OBJECT.SYSTEM_FIXED_FONT:
            case types_js_1.STOCK_OBJECT.DEFAULT_GUI_FONT:
    ...
    applyPen(ctx, state, device) {
        const style = state.pen.style & types_js_1.PEN_STYLE.STYLE_MASK;
        if (style === types_js_1.PEN_STYLE.NULL) {
            return false;
        }
        const scales = this.scaleMagnitudes(state, device);
        const width = state.pen.width > 0 ? state.pen.width * ((scales.sx + scales.sy) / 2) : 1;
        ctx.strokeStyle = (0, types_js_1.colorRefToCss)(state.pen.color);
        ctx.lineWidth = Math.max(width, 0.5);
        ctx.lineCap = 'round';
        ctx.lineJoin = 'round';
        switch (style) {
            case types_js_1.PEN_STYLE.DASH:
                ctx.setLineDash([width * 4, width * 2]);
                break;
            case types_js_1.PEN_STYLE.DOT:
                ctx.setLineDash([width, width * 2]);
                break;
            case types_js_1.PEN_STYLE.DASHDOT:
                ctx.setLineDash([width * 4, width * 2, width, width * 2]);
                break;
            case types_js_1.PEN_STYLE.DASHDOTDOT:
    ...
    applyBrush(ctx, state) {
        if (state.brush.style === types_js_1.BRUSH_STYLE.NULL) {
            return false;
        }
        // BS_HATCHED and BS_PATTERN are approximated as solid foreground fills
        ctx.fillStyle = (0, types_js_1.colorRefToCss)(state.brush.color);
        return true;
    }
    ...
    fillStrokePath(ctx, state, device, buildPath) {
        this.withClip(ctx, state, () => {
            ...
            if (this.applyBrush(ctx, state)) {
                ctx.fill(state.polyFillMode === types_js_1.POLY_FILL_MODE.WINDING ? 'nonzero' : 'evenodd');
            }
            if (this.applyPen(ctx, state, device)) {
                ctx.stroke();
            }
    ...
    drawText(ctx, state, device, record) {
        ...
        ctx.textAlign = 'left';
        const useCurrentPos = (state.textAlign & types_js_1.TEXT_ALIGN.UPDATECP) !== 0;
        ...
        const horizontal = state.textAlign & types_js_1.TEXT_ALIGN.HORIZONTAL_MASK;
        let startX = ref.x;
        if (horizontal === types_js_1.TEXT_ALIGN.RIGHT) {
            startX -= totalWidth;
        }
        else if (horizontal === types_js_1.TEXT_ALIGN.CENTER) {
            startX -= totalWidth / 2;
        }
        const vertical = state.textAlign & types_js_1.TEXT_ALIGN.VERTICAL_MASK;
        let baseline = 'top';
        let baselineY = ref.y + sizePx * 0.8;
        if (vertical === types_js_1.TEXT_ALIGN.BASELINE) {
            baseline = 'alphabetic';
            baselineY = ref.y;
        }
        else if (vertical === types_js_1.TEXT_ALIGN.BOTTOM) {
            baseline = 'bottom';
        ...
        this.withClip(ctx, state, () => {
            ctx.save();
            try {
                if ((record.options & types_js_1.ETO.CLIPPED) !== 0 && record.rect) {
                    const p1 = this.toDevice(state, device, record.rect.left, record.rect.top);
                    const p2 = this.toDevice(state, device, record.rect.right, record.rect.bottom);
                ...
                if ((record.options & types_js_1.ETO.OPAQUE) !== 0 && record.rect) {
                    const p1 = this.toDevice(state, device, record.rect.left, record.rect.top);
                    const p2 = this.toDevice(state, device, record.rect.right, record.rect.bottom);
                    const bg = normalizeRect(p1, p2);
                    ctx.fillStyle = (0, types_js_1.colorRefToCss)(state.bkColor);
                    ctx.fillRect(bg.left, bg.top, bg.right - bg.left, bg.bottom - bg.top);
                }
                else if (state.bkMode === types_js_1.BK_MODE.OPAQUE) {
                    const cellTop = baselineY - sizePx * 0.8;
                    ctx.fillStyle = (0, types_js_1.colorRefToCss)(state.bkColor);
                    ctx.fillRect(startX, cellTop, totalWidth, sizePx * 1.05);
                }
                ctx.fillStyle = (0, types_js_1.colorRefToCss)(state.textColor);
                ...
                if (font.underline || font.strikeOut) {
                    ctx.strokeStyle = (0, types_js_1.colorRefToCss)(state.textColor);
    ...
    drawDib(ctx, state, device, record, decoded) {
        ...
        this.withClip(ctx, state, () => {
            ...
            if (record.rop === types_js_1.ROP.PATCOPY) {
                if (this.applyBrush(ctx, state)) {
                    ctx.fillRect(dest.left, dest.top, destWidth, destHeight);
                }
            }
            else if (record.rop === types_js_1.ROP.BLACKNESS) {
                ctx.fillStyle = 'rgb(0,0,0)';
                ctx.fillRect(dest.left, dest.top, destWidth, destHeight);
            }
            else if (record.rop === types_js_1.ROP.WHITENESS) {
    ...
    async decodeDib(dib) {
        try {
            ...
            const image = await (0, skia_canvas_1.loadImage)(bmp);
...
exports.EmfRenderer = EmfRenderer;
