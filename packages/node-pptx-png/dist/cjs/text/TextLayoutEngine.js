...
exports.TextLayoutEngine = void 0;
exports.resolveLineHeightPx = resolveLineHeightPx;
exports.resolveParagraphSpacingPx = resolveParagraphSpacingPx;
exports.segmentSmallCaps = segmentSmallCaps;
exports.createTextLayoutEngine = createTextLayoutEngine;
const FontResolver_js_1 = require("./FontResolver.js");
const WordWrapper_js_1 = require("./WordWrapper.js");
const BulletFormatter_js_1 = require("./BulletFormatter.js");
const AutofitSolver_js_1 = require("./AutofitSolver.js");
 ...
 */
function resolveLineHeightPx(lineSpacing, singleLineHeightPx, scaleY) {
    if (lineSpacing === undefined) {
        return singleLineHeightPx;
    }
    if (lineSpacing < 0) {
        // Fixed spacing: -lineSpacing is in hundredths of a point
        return (0, FontResolver_js_1.pointsToPixels)(-lineSpacing / 100) * scaleY;
    }
 ...
 */
function resolveParagraphSpacingPx(spacing, singleLineHeightPx, scaleY) {
    ...
    return (0, UnitConverter_js_1.emuToPixels)(spacing) * scaleY;
}
/**
 ...
 */
function segmentSmallCaps(text) {
    const segments = [];
...
class TextLayoutEngine {
    ...
    bulletFormatter;
    constructor(config) {
        this.logger = config.logger ?? (0, Logger_js_1.createLogger)('warn', 'TextLayoutEngine');
        ...
        this.wordWrapper = new WordWrapper_js_1.WordWrapper({
            fontResolver: this.fontResolver,
            logger: this.logger.child?.('WordWrapper'),
        });
        this.bulletFormatter = new BulletFormatter_js_1.BulletFormatter({
    ...
    calculateTextBounds(shapeBounds, bodyProps, scaleX, scaleY) {
        const leftInset = (0, UnitConverter_js_1.emuToPixels)(bodyProps.leftInset ?? DEFAULT_INSET_EMU) * scaleX;
        const rightInset = (0, UnitConverter_js_1.emuToPixels)(bodyProps.rightInset ?? DEFAULT_INSET_EMU) * scaleX;
        const topInset = (0, UnitConverter_js_1.emuToPixels)(bodyProps.topInset ?? DEFAULT_VERTICAL_INSET_EMU) * scaleY;
        const bottomInset = (0, UnitConverter_js_1.emuToPixels)(bodyProps.bottomInset ?? DEFAULT_VERTICAL_INSET_EMU) * scaleY;
        return {
    ...
    layoutParagraph(ctx, paragraph, textBounds, wrapMode, bulletIndex, scaleX, scaleY, lineMetrics, autofit) {
        ...
        const marginLeft = (0, UnitConverter_js_1.emuToPixels)(paraProps.marginLeft ?? 0) * scaleX;
        const indent = (0, UnitConverter_js_1.emuToPixels)(paraProps.indent ?? 0) * scaleX;
        ...
        let totalLeftMargin = marginLeft + (0, UnitConverter_js_1.emuToPixels)(bulletIndent) * scaleX;
        ...
        if (fragments.length === 0) {
            ...
            if (endProps?.fontSize !== undefined) {
                const endFont = this.fontResolver.resolveFont(endProps.fontFamily ?? defaultFont.family, (0, UnitConverter_js_1.fontSizeToPoints)(endProps.fontSize) * autofit.fontScale, endProps.bold ?? false, endProps.italic ?? false, scaleY);
    ...
    buildFragments(paragraph, paraProps, scaleY, fontScale) {
        ...
        for (const run of paragraph.runs) {
            ...
            const fontSize = (0, UnitConverter_js_1.fontSizeToPoints)(mergedProps.fontSize ?? DEFAULT_FONT_SIZE) * fontScale;
    ...
    solveAndLayout(ctx, textBody, textBounds, wrapMode, scaleX, scaleY) {
        ...
        const index = (0, AutofitSolver_js_1.solveAutofitScale)(textBounds.height, measure);
        const candidate = AutofitSolver_js_1.AUTOFIT_CANDIDATES[index] ?? { fontScale: 1, lineSpaceReduction: 0 };
    ...
    getDefaultFont(paraProps, scaleY, fontScale) {
        ...
        const fontSize = (0, UnitConverter_js_1.fontSizeToPoints)(defaultProps.fontSize ?? DEFAULT_FONT_SIZE) * fontScale;
...
exports.TextLayoutEngine = TextLayoutEngine;
/**
 * Creates a TextLayoutEngine instance.
 */
function createTextLayoutEngine(fontScheme, logger) {
