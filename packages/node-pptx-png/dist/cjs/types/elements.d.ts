 ...
 */
export type ElementType = 'shape' | 'picture' | 'textBox' | 'chart' | 'table' | 'groupShape' | 'connectionShape' | 'graphicFrame' | 'oleObject' | 'unknown';
/**
 ...
 */
export type Fill = SolidFill | GradientFill | PatternFill | PictureFill | {
    type: 'none';
};
/**
...
/**
 * Line end decoration shapes (ECMA-376 ST_LineEndType).
 */
export type LineEndType = 'none' | 'triangle' | 'stealth' | 'arrow' | 'diamond' | 'oval';
/**
 * Relative line end decoration sizes (ECMA-376 ST_LineEndWidth / ST_LineEndLength).
 */
export type LineEndSize = 'sm' | 'med' | 'lg';
/**
 * Line end decoration (arrowhead) from a:headEnd / a:tailEnd.
 */
export interface LineEnd {
    /** Decoration shape */
    type: LineEndType;
    /** Size perpendicular to the line, relative to the stroke width */
    width: LineEndSize;
    /** Size along the line, relative to the stroke width */
    length: LineEndSize;
}
/**
...
export interface Stroke {
    ...
    /** Decoration at the start of the line (a:headEnd) */
    headEnd?: LineEnd;
    /** Decoration at the end of the line (a:tailEnd) */
    tailEnd?: LineEnd;
}
/**
 * Shape element (sp).
...
export interface TextRunProperties {
    ...
    baseline?: number;
    /**
     * Character spacing (letter tracking, a:rPr spc) in hundredths of a point:
     * spc="300" adds 3pt to every glyph's advance. May be negative (condensed).
     */
    spacing?: number;
    /**
     * Capitalization override (a:rPr cap, ECMA-376 §21.1.2.3.9): 'all' draws
     * every glyph as uppercase, 'small' draws lowercase letters as reduced-size
     * capitals; 'none' explicitly cancels an inherited value. The stored text
     * is never modified — the transform is applied at layout time.
     */
    caps?: 'all' | 'small' | 'none';
    /**
     * Minimum font size at which pair kerning applies (a:rPr kern, ECMA-376
     * §21.1.2.3.9), in hundredths of a point: kern="1200" kerns runs of 12pt
     * and larger, so PowerPoint leaves small print unkerned. 0 disables
     * kerning outright; absent means no effective value was inherited (the
     * renderer then keeps kerning on, matching shaped-text defaults).
     */
    kerningMin?: number;
}
/**
 ...
 */
export type PlaceholderType = 'title' | 'body' | 'ctrTitle' | 'subTitle' | 'dt' | 'ftr' | 'sldNum' | 'hdr' | 'obj' | 'chart' | 'tbl' | 'clipArt' | 'dgm' | 'media' | 'sldImg' | 'pic';
/**
