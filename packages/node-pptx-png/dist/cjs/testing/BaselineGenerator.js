...
exports.findLibreOfficePath = findLibreOfficePath;
exports.generateBaselines = generateBaselines;
exports.generateBaselinesViaPdf = generateBaselinesViaPdf;
 ...
 */
function execCommand(command, args, timeout) {
 ...
 */
async function generateBaselines(pptxPath, options = {}) {
    ...
    if (!libreOfficePath) {
        return {
            ...
            slideCount: 0,
            success: false,
            error: 'LibreOffice not found. Please install LibreOffice or specify the path via libreOfficePath option.',
            durationMs: Date.now() - startTime,
        };
    }
    ...
    const tempDir = path.join((0, node_os_1.tmpdir)(), `pptimg-baseline-${Date.now()}`);
 ...
 */
async function generateBaselinesViaPdf(pptxPath, options = {}) {
    const startTime = Date.now();
    const opts = { ...DEFAULT_OPTIONS, ...options };
    // Validate PPTX file
    if (!fs.existsSync(pptxPath)) {
...
class BaselineGenerator {
     ...
     */
    async generate(pptxPath, outputDir) {
     ...
     */
    async generateViaPdf(pptxPath, outputDir) {
