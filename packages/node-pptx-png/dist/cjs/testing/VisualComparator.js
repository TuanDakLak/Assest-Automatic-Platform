...
exports.VisualComparator = void 0;
 ...
 */
async function compareImages(baseline, rendered, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    // Load images (narrow the union: skia-canvas v3 declares separate
    // loadImage overloads for paths and buffers)
    const baselineImg = typeof baseline === 'string' ? await (0, skia_canvas_1.loadImage)(baseline) : await (0, skia_canvas_1.loadImage)(baseline);
    const renderedImg = typeof rendered === 'string' ? await (0, skia_canvas_1.loadImage)(rendered) : await (0, skia_canvas_1.loadImage)(rendered);
    ...
    const baselineCanvas = new skia_canvas_1.Canvas(width, height);
    const renderedCanvas = new skia_canvas_1.Canvas(width, height);
    ...
    const pixelDiffPercent = calculatePixelDiffPercent(baselineData.data, renderedData.data, opts.pixelThreshold);
    const ssim = calculateSSIM(baselineData.data, renderedData.data, width, height, opts.ssimWindowSize);
    // Generate diff image if requested
    let diffImage;
    if (opts.generateDiffImage) {
        diffImage = await generateDiffImage(baselineData.data, renderedData.data, width, height, opts.pixelThreshold);
    }
    return {
        ssim,
        mse,
 ...
 */
function calculatePixelDiffPercent(data1, data2, threshold) {
 ...
 */
function calculateSSIM(data1, data2, width, height, windowSize) {
 ...
 */
function calculateCovariance(values1, values2, mean1, mean2) {
 ...
 */
async function generateDiffImage(data1, data2, width, height, threshold) {
    const diffCanvas = new skia_canvas_1.Canvas(width, height);
