...
exports.PptxDocument = void 0;
exports.openPresentation = openPresentation;
const options_js_1 = require("../types/options.js");
const UnitConverter_js_1 = require("./UnitConverter.js");
const PptxParser_js_1 = require("./PptxParser.js");
const ThemeResolver_js_1 = require("../theme/ThemeResolver.js");
const SlideRenderer_js_1 = require("../rendering/SlideRenderer.js");
const FontManager_js_1 = require("../text/FontManager.js");
...
class PptxDocument {
    ...
    docWarnings = new WarningCollector_js_1.WarningCollector();
    closed = false;
    ...
    static async open(input, options = {}, logger) {
        const docLogger = logger ?? (0, Logger_js_1.createLogger)(options.logLevel ?? 'warn', 'PptxDocument');
        const parser = new PptxParser_js_1.PptxParser(docLogger.child('Parser'));
    ...
    async exportPdf(options = {}) {
        ...
        for (const slideNumber of slideNumbers) {
            ...
            const warnings = new WarningCollector_js_1.WarningCollector({ slideNumber });
    ...
    async createRenderSession(options) {
        const merged = (0, options_js_1.mergeRenderOptions)(options);
        ...
        const sessionWarnings = new WarningCollector_js_1.WarningCollector();
        await this.prepareFonts(resolved.fonts, sessionWarnings);
        const theme = await this.resolveThemeOnce();
        const renderer = new SlideRenderer_js_1.SlideRenderer(theme, resolved, this.logger.child('Slide'));
    ...
    resolveTargetSize(options, hasExplicitSize) {
        ...
        if (scale !== undefined) {
            ...
            return {
                ...options,
                width: Math.max(1, Math.round((this.size.widthEmu / UnitConverter_js_1.EMU_PER_PIXEL_96DPI) * scale)),
                height: Math.max(1, Math.round((this.size.heightEmu / UnitConverter_js_1.EMU_PER_PIXEL_96DPI) * scale)),
            };
        }
        if (preset !== undefined) {
            // Runtime lookup tolerates unknown strings from untyped callers
            const presetWidth = options_js_1.SIZE_PRESET_WIDTHS[preset];
            if (presetWidth === undefined) {
                throw new Error(`Invalid preset option: ${String(preset)}. ` +
                    `Expected one of: ${Object.keys(options_js_1.SIZE_PRESET_WIDTHS).join(', ')}.`);
            }
    ...
    async prepareFonts(fonts, sessionWarnings) {
        ...
        (0, FontResolver_js_1.setCustomFontFallbacks)(fonts?.fallbacks);
        ...
        const fontManager = new FontManager_js_1.FontManager({
     ...
     */
    resolveThemeOnce() {
        this.themePromise ??= new ThemeResolver_js_1.ThemeResolver(this.logger.child('Theme')).resolveTheme(this.parser);
    ...
    resolveThemeForSlide(masterPath) {
        ...
        if (!themePromise) {
            themePromise = this.parser
                ...
                ? new ThemeResolver_js_1.ThemeResolver(this.logger.child('Theme')).parseTheme(themeData)
    ...
    rendererForTheme(session, theme) {
        ...
        if (!renderer) {
            renderer = new SlideRenderer_js_1.SlideRenderer(theme, session.options, this.logger.child('Slide'));
    ...
    async renderSlideWithSession(session, slideIndex) {
        ...
        const warnings = new WarningCollector_js_1.WarningCollector({ slideNumber: slideIndex + 1 });
        try {
            // Get slide data
            const slideData = await this.parser.getSlide(slideIndex);
 ...
 * @param options Open options (log level)
 */
async function openPresentation(input, options = {}) {
