...
exports.TableStyleParser = exports.TABLE_STYLES_PART_PATH = void 0;
exports.parseTableGradientFill = parseTableGradientFill;
exports.createTableStyleParser = createTableStyleParser;
 ...
 */
exports.TABLE_STYLES_PART_PATH = 'ppt/tableStyles.xml';
/**
...
class TableStyleParser {
    ...
    colorResolver;
    constructor(config) {
        this.logger = config.logger ?? (0, Logger_js_1.createLogger)('warn', 'TableStyleParser');
    ...
    async load(parser) {
        ...
        let root;
        try {
            if (!parser.fileExists(exports.TABLE_STYLES_PART_PATH)) {
                this.logger.debug('No tableStyles.xml in package');
                return empty;
            }
            const ordered = await parser.readXmlOrdered(exports.TABLE_STYLES_PART_PATH);
            root = (0, PptxParser_js_1.getOrderedChildren)(ordered, ['a:tblStyleLst'])[0]?.node;
        }
        catch (error) {
    ...
    parseTableStyleList(tblStyleLst) {
        ...
        const defaultStyleId = (0, PptxParser_js_1.getXmlAttr)(tblStyleLst, 'def');
        for (const styleNode of (0, PptxParser_js_1.getXmlChildren)(tblStyleLst, 'a:tblStyle')) {
    ...
    parseTableStyle(styleNode) {
        const styleId = (0, PptxParser_js_1.getXmlAttr)(styleNode, 'styleId');
        if (!styleId) {
        ...
        const style = {
            styleId,
            styleName: (0, PptxParser_js_1.getXmlAttr)(styleNode, 'styleName'),
        };
        // a:tblBg (CT_TableBackgroundStyle) carries the same fill/fillRef
        // choice as a:tcStyle
        const tblBg = (0, PptxParser_js_1.getXmlChild)(styleNode, 'a:tblBg');
        if (tblBg && typeof tblBg === 'object') {
            style.tblBg = this.parseThemeableFill(tblBg);
        }
        for (const [tag, key] of STYLE_PART_TAGS) {
            const partNode = (0, PptxParser_js_1.getXmlChild)(styleNode, tag);
    ...
    parsePartStyle(partNode) {
        ...
        const tcStyle = (0, PptxParser_js_1.getXmlChild)(partNode, 'a:tcStyle');
        if (tcStyle && typeof tcStyle === 'object') {
            part.fill = this.parseThemeableFill(tcStyle);
            const tcBdr = (0, PptxParser_js_1.getXmlChild)(tcStyle, 'a:tcBdr');
            if (tcBdr && typeof tcBdr === 'object') {
                ...
                const pick = (specTag, altTag) => (0, PptxParser_js_1.getXmlChild)(tcBdr, specTag) ?? (0, PptxParser_js_1.getXmlChild)(tcBdr, altTag);
                part.borders = {
                    ...
                    insideH: this.parseThemeableLine((0, PptxParser_js_1.getXmlChild)(tcBdr, 'a:insideH')),
                    insideV: this.parseThemeableLine((0, PptxParser_js_1.getXmlChild)(tcBdr, 'a:insideV')),
                    tlToBr: this.parseThemeableLine(pick('a:tl2br', 'a:lnTlToBr')),
                    blToTr: this.parseThemeableLine(pick('a:tr2bl', 'a:lnBlToTr')),
                };
            }
        }
        part.text = this.parseTcTxStyle((0, PptxParser_js_1.getXmlChild)(partNode, 'a:tcTxStyle'));
        return part;
    }
    /**
    ...
    parseThemeableFill(tcStyle) {
        const fillNode = (0, PptxParser_js_1.getXmlChild)(tcStyle, 'a:fill');
        if (fillNode !== undefined) {
            return this.parseDirectFill(fillNode);
        }
        const fillRef = (0, PptxParser_js_1.getXmlChild)(tcStyle, 'a:fillRef');
    ...
    parseDirectFill(fillNode) {
        if (typeof fillNode !== 'object')
            return undefined;
        if ((0, PptxParser_js_1.hasXmlChild)(fillNode, 'a:noFill')) {
            return { type: 'none' };
        }
        const solidFill = (0, PptxParser_js_1.getXmlChild)(fillNode, 'a:solidFill');
        if (solidFill) {
        ...
        const gradFill = (0, PptxParser_js_1.getXmlChild)(fillNode, 'a:gradFill');
    ...
    parseThemeableLine(lineParent) {
        if (!lineParent || typeof lineParent !== 'object')
            return undefined;
        if ((0, PptxParser_js_1.hasXmlChild)(lineParent, 'a:ln')) {
            return this.parseDirectLine((0, PptxParser_js_1.getXmlChild)(lineParent, 'a:ln'));
        }
        if ((0, PptxParser_js_1.hasXmlChild)(lineParent, 'a:lnRef')) {
            return this.resolveLineRef((0, PptxParser_js_1.getXmlChild)(lineParent, 'a:lnRef'));
        }
        return undefined;
    }
    /**
    ...
    parseDirectLine(lnNode) {
        ...
        if (!lnNode || typeof lnNode !== 'object') {
            return { width: DEFAULT_STYLE_BORDER_WIDTH_EMU, color: { ...geometry_js_1.Colors.black } };
        }
        ...
        return { width, color: color ?? { ...geometry_js_1.Colors.black }, dash };
    }
    /**
    ...
    flattenThemeLineStyle(style, phClr) {
        ...
        return {
            ...
            color: color ?? phClr ?? { ...geometry_js_1.Colors.black },
            dash: style.dash,
        };
    }
    /**
    ...
    parseTcTxStyle(txStyleNode) {
        if (!txStyleNode || typeof txStyleNode !== 'object')
            return {};
        const text = {
            bold: parseOnOff((0, PptxParser_js_1.getXmlAttr)(txStyleNode, 'b')),
            italic: parseOnOff((0, PptxParser_js_1.getXmlAttr)(txStyleNode, 'i')),
            // EG_ColorChoice: the color element is a direct child of tcTxStyle
            color: this.colorResolver.resolveColorElement(txStyleNode),
        };
        const fontRef = (0, PptxParser_js_1.getXmlChild)(txStyleNode, 'a:fontRef');
        if (fontRef && typeof fontRef === 'object') {
            text.fontRefIdx = (0, PptxParser_js_1.getXmlAttr)(fontRef, 'idx');
...
exports.TableStyleParser = TableStyleParser;
/**
...
function parseTableGradientFill(gradFill, colorResolver, logger) {
    ...
    for (const gs of (0, PptxParser_js_1.getXmlChildren)(gsLst, 'a:gs')) {
        ...
        const position = Number.isFinite(rawPos)
            ? Math.min(1, Math.max(0, (0, UnitConverter_js_1.percentageToDecimal)(rawPos)))
            : 0;
    ...
    if ((0, PptxParser_js_1.hasXmlChild)(gradFill, 'a:path')) {
    ...
    const angAttr = lin ? (0, PptxParser_js_1.getXmlAttr)(lin, 'ang') : undefined;
 ...
 */
function createTableStyleParser(theme, colorMap, logger) {
