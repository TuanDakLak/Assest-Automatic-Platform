...
exports.RelationshipParser = exports.RelationshipTypes = void 0;
exports.createRelationshipParser = createRelationshipParser;
...
class RelationshipParser {
    ...
    constructor(config) {
        ...
        this.logger = config.logger ?? (0, Logger_js_1.createLogger)('warn', 'RelationshipParser');
    }
    /**
     ...
     */
    async loadRelationships(sourcePath) {
        // Check cache first
     ...
     */
    async resolveRelationshipId(sourcePath, relationshipId) {
     ...
     */
    async resolveImageRelationship(sourcePath, relationshipId) {
        const cached = await this.loadRelationships(sourcePath);
        const rel = cached.relationships.get(relationshipId);
        if (!rel) {
        ...
        if (rel.type !== exports.RelationshipTypes.image) {
            this.logger.warn('Relationship is not an image type', {
                id: relationshipId,
                type: rel.type,
                expected: exports.RelationshipTypes.image,
            });
        }
     ...
     */
    async getImageRelationships(sourcePath) {
        const cached = await this.loadRelationships(sourcePath);
        const images = [];
     ...
     */
    async getRelationship(sourcePath, relationshipId) {
        const cached = await this.loadRelationships(sourcePath);
     ...
     */
    async getAllRelationships(sourcePath) {
        const cached = await this.loadRelationships(sourcePath);
     ...
     */
    async getRelationshipsByType(sourcePath, type) {
        const cached = await this.loadRelationships(sourcePath);
        const result = [];
 ...
 */
function createRelationshipParser(parser, logger) {
