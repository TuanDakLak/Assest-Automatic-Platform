...
const WordWrapper_js_1 = require("../text/WordWrapper.js");
...
function classifyScriptChar(char) {
    if ((0, WordWrapper_js_1.isCjkChar)(char))
        return 'ea';
...
class TextParser {
    ...
    slideNumber;
    constructor(config) {
        this.logger = config.logger ?? (0, Logger_js_1.createLogger)('warn', 'TextParser');
    ...
    parseTextBody(txBody, inheritedListStyles) {
        ...
        const lstStyle = (0, PptxParser_js_1.getXmlChild)(txBody, 'a:lstStyle');
        // Parse paragraphs
    ...
    parseBodyProperties(bodyPr) {
        ...
        const anchorCtr = (0, PptxParser_js_1.getXmlAttr)(bodyPr, 'anchorCtr') === '1';
        // Parse wrap
        const wrapAttr = (0, PptxParser_js_1.getXmlAttr)(bodyPr, 'wrap');
        ...
        // Parse rotation
        const rotAttr = (0, PptxParser_js_1.getXmlAttr)(bodyPr, 'rot');
    ...
    parsePercentAttr(node, attr) {
        const raw = (0, PptxParser_js_1.getXmlAttr)(node, attr);
        if (raw === undefined)
            return undefined;
    ...
    parseParagraph(pNode, lstStyle, inheritedListStyles) {
        ...
        const pPr = (0, PptxParser_js_1.getXmlChild)(pNode, 'a:pPr');
        ...
        const runs = [];
        const orderedChildren = (0, PptxParser_js_1.getChildrenInDocumentOrder)(pNode, 'a:p', ['a:r', 'a:fld', 'a:br']);
        ...
        const endParaRPr = (0, PptxParser_js_1.getXmlChild)(pNode, 'a:endParaRPr');
    ...
    parseLineBreak(brNode, defaultProps) {
        const rPr = (0, PptxParser_js_1.getXmlChild)(brNode, 'a:rPr');
    ...
    parseParagraphProperties(pPr, lstStyle, inheritedListStyles) {
        ...
        const lvlAttr = pPr ? (0, PptxParser_js_1.getXmlAttr)(pPr, 'lvl') : undefined;
        ...
        const sources = [
            ...(inheritedListStyles ?? []).map((style) => (0, PptxParser_js_1.getXmlChild)(style, levelKey)),
            lstStyle ? (0, PptxParser_js_1.getXmlChild)(lstStyle, levelKey) : undefined,
            pPr,
        ];
        let merged = {};
    ...
    parsePPrNode(node) {
        // Parse alignment
        const algn = (0, PptxParser_js_1.getXmlAttr)(node, 'algn');
        let alignment;
        if (algn === 'l')
            alignment = 'left';
        else if (algn === 'ctr')
        ...
        // Parse spacing
        const spaceBefore = this.parseSpacing((0, PptxParser_js_1.getXmlChild)(node, 'a:spcBef'));
        const spaceAfter = this.parseSpacing((0, PptxParser_js_1.getXmlChild)(node, 'a:spcAft'));
        const lineSpacing = this.parseLineSpacing((0, PptxParser_js_1.getXmlChild)(node, 'a:lnSpc'));
        // Parse bullet aspects
        const bullet = this.parseBulletProps(node);
        // Parse default run properties
        const defRPr = (0, PptxParser_js_1.getXmlChild)(node, 'a:defRPr');
    ...
    parseBulletProps(node) {
        ...
        if (this.getPresentChild(node, 'a:buNone')) {
            ...
            if (buAutoNum) {
                props.type = 'auto';
                props.autoNumType = (0, PptxParser_js_1.getXmlAttr)(buAutoNum, 'type') ?? 'arabicPeriod';
                const startAtAttr = (0, PptxParser_js_1.getXmlAttr)(buAutoNum, 'startAt');
                ...
                props.type = 'char';
                props.char = (0, PptxParser_js_1.getXmlAttr)(buChar, 'char') ?? '\u2022';
                hasAny = true;
            }
            else if (buBlip) {
                props.type = 'picture';
        ...
        // Bullet color
        const buClr = (0, PptxParser_js_1.getXmlChild)(node, 'a:buClr');
        ...
        // Bullet size
        const buSzPct = (0, PptxParser_js_1.getXmlChild)(node, 'a:buSzPct');
        if (buSzPct) {
            props.sizePercent = parseInt((0, PptxParser_js_1.getXmlAttr)(buSzPct, 'val') ?? '100000', 10) / 1000;
            hasAny = true;
        }
        // Bullet font
        const buFont = (0, PptxParser_js_1.getXmlChild)(node, 'a:buFont');
        const font = buFont ? (0, PptxParser_js_1.getXmlAttr)(buFont, 'typeface') : undefined;
        if (font) {
            props.font = font;
            hasAny = true;
        }
    ...
    parseTextRun(rNode, defaultProps) {
        ...
        const rPr = (0, PptxParser_js_1.getXmlChild)(rNode, 'a:rPr');
        const runProps = rPr ? this.parseRunProperties(rPr) : undefined;
    ...
    parseTextField(fldNode, defaultProps) {
        ...
        const tNode = (0, PptxParser_js_1.getXmlChild)(fldNode, 'a:t');
        let text = tNode ? this.extractTextContent(tNode) : '';
        const fieldType = (0, PptxParser_js_1.getXmlAttr)(fldNode, 'type');
        if (fieldType === 'slidenum' && this.slideNumber !== undefined) {
            text = String(this.slideNumber);
        }
        // Parse run properties
        const rPr = (0, PptxParser_js_1.getXmlChild)(fldNode, 'a:rPr');
        const runProps = rPr ? this.parseRunProperties(rPr) : undefined;
    ...
    parseRunProperties(rPr) {
        ...
        const szAttr = (0, PptxParser_js_1.getXmlAttr)(rPr, 'sz');
        const fontSize = szAttr ? parseInt(szAttr, 10) : undefined;
        // Parse bold
        const bAttr = (0, PptxParser_js_1.getXmlAttr)(rPr, 'b');
        const bold = bAttr === '1' || bAttr === 'true'
            ...
            : bAttr === '0' || bAttr === 'false'
                ...
                : undefined;
        // Parse italic
        const iAttr = (0, PptxParser_js_1.getXmlAttr)(rPr, 'i');
        ...
        const spcAttr = (0, PptxParser_js_1.getXmlAttr)(rPr, 'spc');
        ...
        const kernAttr = (0, PptxParser_js_1.getXmlAttr)(rPr, 'kern');
        ...
        const capAttr = (0, PptxParser_js_1.getXmlAttr)(rPr, 'cap');
        const caps = capAttr === 'all' || capAttr === 'small' || capAttr === 'none' ? capAttr : undefined;
        // Parse font family
        const latin = (0, PptxParser_js_1.getXmlChild)(rPr, 'a:latin');
        const ea = (0, PptxParser_js_1.getXmlChild)(rPr, 'a:ea');
        const cs = (0, PptxParser_js_1.getXmlChild)(rPr, 'a:cs');
        ...
        const latinTypeface = latin ? (0, PptxParser_js_1.getXmlAttr)(latin, 'typeface') : undefined;
        ...
        const eaTypeface = ea ? (0, PptxParser_js_1.getXmlAttr)(ea, 'typeface') : undefined;
        const csTypeface = cs ? (0, PptxParser_js_1.getXmlAttr)(cs, 'typeface') : undefined;
        const eastAsianFontFamily = eaTypeface === '' ? undefined : eaTypeface;
        const complexScriptFontFamily = csTypeface === '' ? undefined : csTypeface;
        // Parse color
        const solidFill = (0, PptxParser_js_1.getXmlChild)(rPr, 'a:solidFill');
        const color = solidFill ? this.colorResolver.resolveColorElement(solidFill) : undefined;
        // Parse scheme color reference
        const schemeClr = solidFill ? (0, PptxParser_js_1.getXmlChild)(solidFill, 'a:schemeClr') : undefined;
        const schemeColor = schemeClr ? (0, PptxParser_js_1.getXmlAttr)(schemeClr, 'val') : undefined;
        if (schemeColor) {
    ...
    parseSpacing(spcNode) {
        if (!spcNode)
            return undefined;
        // Check for points
        const spcPts = (0, PptxParser_js_1.getXmlChild)(spcNode, 'a:spcPts');
        if (spcPts) {
            const val = (0, PptxParser_js_1.getXmlAttr)(spcPts, 'val');
            if (!val)
                return undefined;
        ...
        const spcPct = (0, PptxParser_js_1.getXmlChild)(spcNode, 'a:spcPct');
        if (spcPct) {
            // Return as negative to indicate percentage mode
            const val = (0, PptxParser_js_1.getXmlAttr)(spcPct, 'val');
    ...
    parseLineSpacing(lnSpc) {
        ...
        const spcPct = (0, PptxParser_js_1.getXmlChild)(lnSpc, 'a:spcPct');
        if (spcPct) {
            const val = (0, PptxParser_js_1.getXmlAttr)(spcPct, 'val');
            return val ? parseInt(val, 10) : undefined;
        }
        // Check for points (treat as fixed line height)
        const spcPts = (0, PptxParser_js_1.getXmlChild)(lnSpc, 'a:spcPts');
        if (spcPts) {
            const val = (0, PptxParser_js_1.getXmlAttr)(spcPts, 'val');
    ...
    parseIntAttr(node, attr) {
        const value = (0, PptxParser_js_1.getXmlAttr)(node, attr);
 ...
 */
function createTextParser(theme, logger) {
