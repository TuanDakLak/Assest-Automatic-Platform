...
import { EMU_PER_PIXEL_96DPI } from '../core/UnitConverter.js';
 ...
 * CLI's usage error.
 */
export function parseSlideList(value) {
    const slides = [];
...
async function runRenderPptx(rawArgs) {
    ...
    const document = await openPresentation(inputPath, { logLevel: 'error' });
    const rendered = [];
    const failed = [];
    const warnings = [];
    try {
    ...
    try {
        return JSON.stringify({
            ...
            size: {
                ...
                widthPx: Math.round(document.size.widthEmu / EMU_PER_PIXEL_96DPI),
                heightPx: Math.round(document.size.heightEmu / EMU_PER_PIXEL_96DPI),
            },
        }, null, 2);
    }
    finally {
    ...
    async handleLine(line) {
        ...
        try {
            ...
            return {
                jsonrpc: '2.0',
                id: null,
                error: { code: JSONRPC_ERRORS.PARSE_ERROR, message: 'Parse error: invalid JSON' },
            };
        }
    ...
    async handleMessage(message) {
        ...
        if (typeof method !== 'string') {
            ...
            return {
                jsonrpc: '2.0',
                id,
                error: { code: JSONRPC_ERRORS.INVALID_REQUEST, message: 'Invalid request: missing method' },
            };
        }
        ...
        try {
            ...
            if (error instanceof McpInvalidParamsError) {
                return {
                    jsonrpc: '2.0',
                    id,
                    error: { code: JSONRPC_ERRORS.INVALID_PARAMS, message: error.message },
                };
            }
            ...
            return {
                jsonrpc: '2.0',
                id,
                error: { code: JSONRPC_ERRORS.INTERNAL_ERROR, message: messageText },
            };
        }
    }
