 ...
 */
import { getXmlAttr, hasXmlChild } from '../core/PptxParser.js';
 ...
 */
export function computeShadowOffset(distance, directionDegrees) {
...
function parseIntAttr(node, name, fallback) {
    const raw = getXmlAttr(node, name);
    if (raw === undefined)
        return fallback;
...
function parsePercentAttr(node, name, fallbackDecimal) {
    return percentageToDecimal(parseIntAttr(node, name, fallbackDecimal * 100000));
}
/**
...
export class EffectParser {
    ...
    colorResolver;
    constructor(config) {
        this.logger = config.logger ?? createLogger('warn', 'EffectParser');
    ...
    parseShapeEffects(spPr) {
        if (!spPr || !hasXmlChild(spPr, 'a:effectLst')) {
            return undefined;
        }
    ...
    parseOuterShadow(outerShdw) {
        const alignment = getXmlAttr(outerShdw, 'algn');
        return {
            type: 'outerShadow',
    ...
    parseReflection(reflection) {
        const alignment = getXmlAttr(reflection, 'algn');
        return {
            type: 'reflection',
 ...
 */
export function createEffectParser(theme, logger) {
