...
import { getXmlChild, getXmlChildren, getXmlAttr, trimTagValueExceptText, trimAttributeValue, } from '../core/PptxParser.js';
...
export class ChartParser {
    ...
    constructor(config = {}) {
        ...
        this.xmlParser = new XMLParser({
            ...
            trimValues: false,
            tagValueProcessor: trimTagValueExceptText,
            attributeValueProcessor: trimAttributeValue,
            isArray: (name) => {
    ...
    determineChartType(plotArea) {
        ...
        for (const tag of ChartParser.SUPPORTED_CHART_TAGS) {
            for (const node of getXmlChildren(plotArea, tag)) {
    ...
    getSecondaryValAxIds(plotArea) {
        const valAxNodes = getXmlChildren(plotArea, 'c:valAx');
        ...
        const hasCategoryAxis = getXmlChildren(plotArea, 'c:catAx').length > 0 ||
            getXmlChildren(plotArea, 'c:dateAx').length > 0;
        const primaryValAxCount = hasCategoryAxis ? 1 : 2;
        const ids = new Set();
        for (let i = primaryValAxCount; i < valAxNodes.length; i++) {
            const id = getXmlAttr(getXmlChild(valAxNodes[i], 'c:axId'), 'val');
            if (id !== undefined)
                ids.add(id);
        }
        return ids;
    ...
    referencesSecondaryAxis(node, secondaryValAxIds) {
        if (secondaryValAxIds.size === 0)
            return false;
        return getXmlChildren(node, 'c:axId').some((axId) => {
            const id = getXmlAttr(axId, 'val');
    ...
    parseDoughnutChart(doughnutChart) {
        const seriesNodes = getXmlChildren(doughnutChart, 'c:ser');
        ...
        const holeSizeNode = getXmlChild(doughnutChart, 'c:holeSize');
        const holeSizeVal = parseInt(getXmlAttr(holeSizeNode, 'val') ?? '', 10);
    ...
    parseScatterChart(scatterChart) {
        const seriesNodes = getXmlChildren(scatterChart, 'c:ser');
        ...
        const styleVal = getXmlAttr(getXmlChild(scatterChart, 'c:scatterStyle'), 'val');
    ...
    parseBubbleChart(bubbleChart) {
        const seriesNodes = getXmlChildren(bubbleChart, 'c:ser');
        const series = seriesNodes.map((ser, index) => {
            const base = this.parseXYSeries(ser, index);
            const bubbleSizes = this.extractValues(getXmlChild(ser, 'c:bubbleSize'));
    ...
    parseXYSeries(ser, index) {
        ...
        const values = this.extractValues(getXmlChild(ser, 'c:yVal'));
        const xVal = getXmlChild(ser, 'c:xVal');
    ...
    extractXValCategories(seriesNodes) {
        const firstXVal = seriesNodes[0] ? getXmlChild(seriesNodes[0], 'c:xVal') : undefined;
    ...
    extractStringCategories(source) {
        const strRef = getXmlChild(source, 'c:strRef');
        const cache = strRef ? getXmlChild(strRef, 'c:strCache') : getXmlChild(source, 'c:strLit');
        if (!cache)
            return [];
        const points = getXmlChildren(cache, 'c:pt');
        const categories = [];
    ...
    extractValues(val) {
        ...
        const cache = numRef ? getXmlChild(numRef, 'c:numCache') : getXmlChild(val, 'c:numLit');
        if (!cache)
            return [];
        const points = getXmlChildren(cache, 'c:pt');
        const values = [];
    ...
    getPointIndex(pt, fallback) {
        const idx = parseInt(getXmlAttr(pt, 'idx') ?? '', 10);
    ...
    findCategoryCache(cat) {
        ...
        if (strRef)
            return getXmlChild(strRef, 'c:strCache');
        const numRef = getXmlChild(cat, 'c:numRef');
        if (numRef)
            return getXmlChild(numRef, 'c:numCache');
        return getXmlChild(cat, 'c:strLit') ?? getXmlChild(cat, 'c:numLit');
    }
    /**
    ...
    extractSeriesColor(ser, index) {
        const idxAttr = getXmlAttr(getXmlChild(ser, 'c:idx'), 'val');
 ...
 */
export function createChartParser(theme, logger) {
