...
import { createGuideContext, evaluateGuideList, ooxmlAngleToRadians, resolveGuideValue, } from './GuideEvaluator.js';
 ...
 */
export function ooxmlArcToBeziers(startX, startY, rx, ry, stAng, swAng) {
    const start = { x: startX, y: startY };
    if (swAng === 0 || rx <= 0 || ry <= 0) {
        return { segments: [], end: start };
    }
    const theta1 = ooxmlAngleToRadians(stAng);
    const theta2 = theta1 + ooxmlAngleToRadians(swAng);
...
function createDefinitionContext(def, space, adjustValues) {
    const ctx = createGuideContext(space.w, space.h, def.av);
    if (adjustValues) {
 ...
 */
export function buildGeometryPaths(def, bounds, adjustValues, guideSpace) {
    ...
    for (const pathDef of def.paths) {
        ...
        const toX = (v) => bounds.x + resolveGuideValue(v, ctx) * sx;
        const toY = (v) => bounds.y + resolveGuideValue(v, ctx) * sy;
        ...
        for (const cmd of pathDef.cmds) {
            switch (cmd[0]) {
                ...
                case 'C':
                    ...
                    break;
                case 'A': {
                    const rx = resolveGuideValue(cmd[1], ctx) * sx;
                    const ry = resolveGuideValue(cmd[2], ctx) * sy;
                    const stAng = resolveGuideValue(cmd[3], ctx);
                    const swAng = resolveGuideValue(cmd[4], ctx);
 ...
 */
export function computeTextRect(def, bounds, adjustValues, guideSpace) {
    if (!def.rect)
        return undefined;
    const space = guideSpace ?? { w: bounds.width, h: bounds.height };
    try {
        ...
        const left = bounds.x + resolveGuideValue(def.rect[0], ctx) * sx;
        const top = bounds.y + resolveGuideValue(def.rect[1], ctx) * sy;
        const right = bounds.x + resolveGuideValue(def.rect[2], ctx) * sx;
        const bottom = bounds.y + resolveGuideValue(def.rect[3], ctx) * sy;
