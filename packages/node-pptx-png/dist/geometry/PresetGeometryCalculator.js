...
import { buildGeometryPaths, computeTextRect } from './GeometryEngine.js';
import { PRESET_SHAPE_DEFINITIONS } from './presetShapeDefinitions.js';
/**
...
export class PresetGeometryCalculator {
    ...
    isSupported(presetName) {
        return (this.isHandCoded(presetName) ||
            Object.prototype.hasOwnProperty.call(PRESET_SHAPE_DEFINITIONS, presetName));
    }
    /**
    ...
    createDefinitionPaths(presetName, bounds, adjustValues) {
        const definition = PRESET_SHAPE_DEFINITIONS[presetName];
        if (!definition)
            return undefined;
        ...
        if (bounds.width <= 0 && bounds.height <= 0)
            return undefined;
        try {
            const paths = buildGeometryPaths(definition, bounds, adjustValues);
        ...
        switch (presetName) {
            ...
            default: {
                ...
                if (!this.isHandCoded(presetName)) {
                    const definition = PRESET_SHAPE_DEFINITIONS[presetName];
                    if (definition) {
                        const textRect = computeTextRect(definition, bounds, adjustValues);
                        if (textRect)
                            return textRect;
                    }
                }
...
//# sourceMappingURL=PresetGeometryCalculator.js.map