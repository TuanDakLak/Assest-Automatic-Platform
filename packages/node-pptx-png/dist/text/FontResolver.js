...
import { getRegisteredFontFamilies } from './FontManager.js';
/**
 ...
 */
export function setCustomFontFallbacks(fallbacks) {
    sessionFallbacks = { ...(fallbacks ?? {}) };
}
/**
 * Returns the current session custom font fallback chains.
 */
export function getCustomFontFallbacks() {
...
function defaultFontAvailability(family) {
    if (getRegisteredFontFamilies().has(family)) {
        return true;
    }
    try {
 ...
 */
export function createFontResolver(fontScheme = DEFAULT_FONT_SCHEME, logger) {
