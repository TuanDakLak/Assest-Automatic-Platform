 ...
 */
export const AUTOFIT_CANDIDATES = FONT_SCALE_STEPS.map((scale) => ({
    fontScale: scale,
 ...
 */
export const AUTOFIT_FLOOR_INDEX = AUTOFIT_CANDIDATES.length - 1;
/**
 ...
 */
export const FIT_EPSILON_PX = 0.01;
/**
 ...
 */
export function solveAutofitScale(availableHeight, measure) {
    const fits = (index) => {
        const candidate = AUTOFIT_CANDIDATES[index];
        if (!candidate)
            return false;
        return measure(candidate, index) <= availableHeight + FIT_EPSILON_PX;
    };
    ...
    return best >= 0 ? best : AUTOFIT_FLOOR_INDEX;
}
//# sourceMappingURL=AutofitSolver.js.map