import { DEFAULT_OFFICE_COLORS, DEFAULT_COLOR_MAP } from '../types/theme.js';
...
export function parseColorMap(clrMapNode) {
    if (!clrMapNode)
        return undefined;
    const readSlot = (attr) => {
        const value = getXmlAttr(clrMapNode, attr);
        if (value !== undefined && THEME_COLOR_SLOTS.has(value)) {
            return value;
        }
        return DEFAULT_COLOR_MAP[attr];
    };
    return {
        bg1: readSlot('bg1'),
        tx1: readSlot('tx1'),
...
function parseColorMapOverride(node) {
    const clrMapOvr = node ? getXmlChild(node, 'p:clrMapOvr') : undefined;
    const override = clrMapOvr ? getXmlChild(clrMapOvr, 'a:overrideClrMapping') : undefined;
 ...
 */
export function resolveEffectiveColorMap(masterNode, layoutNode, slideNode) {
    ...
    const masterMap = parseColorMap(masterNode ? getXmlChild(masterNode, 'p:clrMap') : undefined);
    return masterMap ?? { ...DEFAULT_COLOR_MAP };
}
/**
...
export class ColorResolver {
    colorScheme;
    colorMap;
    constructor(colorScheme = DEFAULT_OFFICE_COLORS, colorMap = DEFAULT_COLOR_MAP) {
    ...
    resolveSchemeColor(schemeColorType, phClr) {
        switch (schemeColorType) {
            ...
            case 'phClr':
                ...
                return phClr ? { ...phClr } : { ...Colors.black };
            default:
                return { ...Colors.black };
        }
    }
    ...
    resolveColorElement(node, phClr) {
        ...
        if (schemeColor) {
            const val = getXmlAttr(schemeColor, 'val');
            if (val) {
