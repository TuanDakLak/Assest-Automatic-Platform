 ...
 */
import { getXmlChild, getXmlChildren, getXmlAttr, getOrderedChildren, getChildrenInDocumentOrder, } from '../core/PptxParser.js';
 ...
 */
export const DIAGRAM_GRAPHIC_DATA_URI = 'http://schemas.openxmlformats.org/drawingml/2006/diagram';
/**
...
export class DiagramRenderer {
    ...
    warnings;
    constructor(config) {
        this.logger = config.logger ?? createLogger('warn', 'DiagramRenderer');
    ...
    async renderDiagram(ctx, graphicData, bounds, frameExtEmu) {
        const relIds = getXmlChild(graphicData, 'dgm:relIds');
        const dataRelId = relIds ? getXmlAttr(relIds, 'r:dm') : undefined;
        if (!dataRelId) {
    ...
    async renderDiagramShape(ctx, shapeRenderer, entry, scaleX, scaleY) {
        ...
        const spPr = getXmlChild(pSp, 'p:spPr');
        ...
        const txBody = getXmlChild(pSp, 'p:txBody');
    ...
    parseTextXfrm(entry, shapeXfrm) {
        const txXfrmNode = getXmlChild(entry.node, 'dsp:txXfrm');
    ...
    async readDataModelExtRelId(dataPath) {
        ...
        const dataModel = getOrderedChildren(ordered, ['dgm:dataModel'])[0]?.node;
        const extLst = dataModel ? getXmlChild(dataModel, 'dgm:extLst') : undefined;
        if (!extLst) {
            return undefined;
        }
        for (const ext of getXmlChildren(extLst, 'a:ext')) {
            const dataModelExt = getXmlChild(ext, 'dsp:dataModelExt');
            const relId = dataModelExt ? getXmlAttr(dataModelExt, 'relId') : undefined;
            if (relId) {
                return relId;
            }
        }
    ...
    async readDrawingShapes(drawingPath) {
        ...
        const entries = [];
        for (const el of getOrderedChildren(spTree, ['dsp:sp', 'dsp:grpSp'])) {
    ...
    collectGroupShapes(grpSpNode, parentTransform, out) {
        const nvGrpSpPr = getXmlChild(grpSpNode, 'dsp:nvGrpSpPr');
        const cNvPr = nvGrpSpPr ? getXmlChild(nvGrpSpPr, 'dsp:cNvPr') : undefined;
        if (cNvPr && getXmlAttr(cNvPr, 'hidden') === '1') {
        ...
        const groupTransform = ownTransform
            ...
            : parentTransform;
        for (const el of getChildrenInDocumentOrder(grpSpNode, 'dsp:grpSp', ['dsp:sp', 'dsp:grpSp'])) {
            if (el.tagName === 'dsp:grpSp') {
...
function buildXfrmNode(transform) {
    ...
    if (transform.rotation !== 0) {
        xfrm['@_rot'] = String(Math.round(transform.rotation * ANGLE_UNIT_PER_DEGREE));
    }
    if (transform.flipH) {
        xfrm['@_flipH'] = '1';
    }
 ...
 */
export function createDiagramRenderer(config) {
