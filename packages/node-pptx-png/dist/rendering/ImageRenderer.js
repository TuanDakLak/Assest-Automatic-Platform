 ...
 */
export function computeStretchDestRect(bounds, fillRect) {
...
export class ImageRenderer {
    ...
    parseBlipFill(blipFill) {
        ...
        const alphaModFix = getXmlChild(blip, 'a:alphaModFix');
        if (alphaModFix) {
    ...
    async renderPictureElement(ctx, picNode, transform) {
        ...
        const cNvPr = getXmlChild(nvPicPr, 'p:cNvPr');
        if (getXmlAttr(cNvPr, 'hidden') === '1') {
 ...
 */
export function createImageRenderer(parser, sourcePath, scaleX, scaleY, logger) {
    return new ImageRenderer({
        parser,
        sourcePath,
        scaleX,
