...
import { parseTableGradientFill } from '../parsers/TableStyleParser.js';
 ...
 */
export function isGradientCellFill(fill) {
...
export class TableRenderer {
    ...
    parseTableProperties(tblPr) {
        if (!tblPr)
            return {};
        const flag = (name) => {
            const value = getXmlAttr(tblPr, name);
    ...
    parseCellBackground(tcPr) {
        ...
        // leak through.
        const gradFill = getXmlChild(tcPr, 'a:gradFill');
        if (gradFill && typeof gradFill === 'object') {
            const fill = parseTableGradientFill(gradFill, this.colorResolver, this.logger);
            if (!fill)
                return 'none';
            return fill.type === 'solid' ? fill.color : fill;
        }
        if (hasXmlChild(tcPr, 'a:gradFill') ||
            hasXmlChild(tcPr, 'a:pattFill') ||
            hasXmlChild(tcPr, 'a:blipFill')) {
            return 'none';
        }
        return undefined;
    }
    ...
    parseCellBorders(tcPr) {
        ...
        borders.tlToBr = this.parseBorder(getXmlChild(tcPr, 'a:lnTlToBr'));
        borders.blToTr = this.parseBorder(getXmlChild(tcPr, 'a:lnBlToTr'));
        return borders;
    }
    /**
    ...
    createLinearGradient(ctx, fill, rect) {
        ...
        const { x0, y0, x1, y1 } = computeLinearGradientPoints(centerX, centerY, halfDiagonal, fill.angle);
 ...
 */
export function mirrorRectHorizontally(rect, tableBounds) {
    return {
 ...
 */
export function createTableRenderer(theme, scaleX, scaleY, logger) {
