 ...
 */
import { getXmlChild, getXmlAttr, getXmlChildren, hasXmlChild } from '../core/PptxParser.js';
import { emuToPixels } from '../core/UnitConverter.js';
import { PlaceholderResolver } from '../core/PlaceholderResolver.js';
import { ColorResolver, resolveEffectiveColorMap } from '../theme/ColorResolver.js';
import { TransformCalculator, } from '../geometry/TransformCalculator.js';
import { PresetGeometryCalculator } from '../geometry/PresetGeometryCalculator.js';
import { buildGeometryPaths, computeTextRect, } from '../geometry/GeometryEngine.js';
import { parseCustomGeometryDefinition } from '../geometry/CustomGeometryParser.js';
import { FillRenderer } from './FillRenderer.js';
import { StrokeRenderer, DEFAULT_STROKE_WIDTH_EMU } from './StrokeRenderer.js';
import { TextRenderer } from './TextRenderer.js';
import { TextParser } from '../parsers/TextParser.js';
import { EffectParser, computeShadowOffset, } from '../parsers/EffectParser.js';
...
export class ShapeRenderer {
    ...
    constructor(config) {
        ...
        this.defaultTextStyle = config.presentationNode
            ? getXmlChild(config.presentationNode, 'p:defaultTextStyle')
            : undefined;
        this.colorMap = resolveEffectiveColorMap(config.masterNode, config.layoutNode, config.slideNode);
        this.colorResolver = new ColorResolver(config.theme.colors, this.colorMap);
        this.transformCalculator = new TransformCalculator();
        this.geometryCalculator = new PresetGeometryCalculator();
        this.placeholderResolver = new PlaceholderResolver(this.logger.child?.('Placeholder'));
    ...
    createSilhouetteSource(paths, bounds, stroke, fill) {
        ...
        if (stroke) {
            silhouetteSource.strokeWidthPx = Math.max(emuToPixels(stroke.width) * Math.min(this.scaleX, this.scaleY), 0.5);
        }
    ...
    createShapePaths(geometryType, bounds, adjustValues, customGeometry) {
        if (customGeometry) {
            try {
                const paths = buildGeometryPaths(customGeometry.def, bounds, undefined, customGeometry.space);
                if (paths.length > 0) {
                    return paths;
                }
            }
    ...
    getShapeTextBounds(geometryType, bounds, adjustValues, customGeometry) {
        if (customGeometry) {
            return computeTextRect(customGeometry.def, bounds, undefined, customGeometry.space) ?? bounds;
        }
    ...
    parseShapeStyle(shapeNode) {
        const styleNode = getXmlChild(shapeNode, 'p:style');
        if (!styleNode)
            return undefined;
        const style = {};
        const fillRef = getXmlChild(styleNode, 'a:fillRef');
        if (fillRef) {
            style.fill = this.resolveStyleFill(fillRef);
        }
        const lnRef = getXmlChild(styleNode, 'a:lnRef');
        if (lnRef) {
            style.stroke = this.resolveStyleStroke(lnRef);
        }
        const fontRef = getXmlChild(styleNode, 'a:fontRef');
        if (fontRef) {
            style.fontRefIdx = getXmlAttr(fontRef, 'idx');
            style.fontColor = this.colorResolver.resolveColorElement(fontRef);
        }
        const effectRef = getXmlChild(styleNode, 'a:effectRef');
        if (effectRef) {
    ...
    resolveStyleStroke(lnRef) {
        ...
        if (!lineStyle) {
            ...
            return phClr ? { width: DEFAULT_STROKE_WIDTH_EMU, color: phClr } : undefined;
        }
    ...
    resolveStyleEffect(effectRef) {
        const idx = parseInt(getXmlAttr(effectRef, 'idx') ?? '0', 10);
        if (!Number.isFinite(idx) || idx <= 0) {
            return undefined;
        }
    ...
    setOuterShadow(ctx, shadow) {
        const { dx, dy } = computeShadowOffset(shadow.distance, shadow.direction);
        ctx.shadowColor = this.colorResolver.rgbaToCss(shadow.color);
        ctx.shadowBlur = emuToPixels(shadow.blurRadius) * Math.min(this.scaleX, this.scaleY);
        ctx.shadowOffsetX = emuToPixels(dx) * this.scaleX;
        ctx.shadowOffsetY = emuToPixels(dy) * this.scaleY;
    }
    /**
    ...
    parseGeometry(spPr) {
        ...
        if (custGeom) {
            const def = parseCustomGeometryDefinition(custGeom);
            if (def) {
                return {
    ...
    getGuideSpace(spPr) {
        ...
        const ext = xfrm ? getXmlChild(xfrm, 'a:ext') : undefined;
        if (!ext)
            return undefined;
        const w = parseInt(getXmlAttr(ext, 'cx') ?? '0', 10);
        const h = parseInt(getXmlAttr(ext, 'cy') ?? '0', 10);
    ...
    getPictureOuterShadow(picNode, spPr) {
        ...
        if (blipFill && typeof blipFill === 'object' && hasXmlChild(blipFill, 'a:tile')) {
            return undefined;
        }
 ...
 */
export function createShapeRenderer(theme, scaleX, scaleY, logger, parser, sourcePath) {
