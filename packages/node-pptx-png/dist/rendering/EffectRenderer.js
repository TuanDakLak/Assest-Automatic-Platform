...
import { computeShadowOffset, } from '../parsers/EffectParser.js';
 ...
 */
export function resolveAlignmentAnchor(alignment, bounds) {
    const left = bounds.x;
    const top = bounds.y;
 ...
 */
export function computeSilhouetteTransform(params, bounds) {
...
export class EffectRenderer {
    ...
    scaleY;
    constructor(config) {
        this.logger = config.logger ?? createLogger('warn', 'EffectRenderer');
    ...
    renderOuterShadow(ctx, source, shadow) {
        ...
        const matrix = computeSilhouetteTransform({
            ...
            skewX: shadow.skewX,
            skewY: shadow.skewY,
            offsetX: emuToPixels(dx) * this.scaleX,
            offsetY: emuToPixels(dy) * this.scaleY,
    ...
    renderInnerShadow(ctx, source, shadow) {
        const blurPx = this.effectLengthToPx(shadow.blurRadius);
        const { dx, dy } = computeShadowOffset(shadow.distance, shadow.direction);
        const offsetX = emuToPixels(dx) * this.scaleX;
        const offsetY = emuToPixels(dy) * this.scaleY;
    ...
    renderWithSoftEdge(ctx, source, softEdge, drawContent) {
        ...
        const finish = () => {
            const mask = new Canvas(layer.canvas.width, layer.canvas.height);
    ...
    renderReflection(ctx, source, reflection, drawContent) {
        ...
        const { dx, dy } = computeShadowOffset(reflection.distance, reflection.direction);
        const matrix = computeSilhouetteTransform({
    ...
    paintSilhouette(ctx, source, cssColor) {
        ...
        if (fillable.length > 0) {
            ...
            for (const path of fillable) {
                applyPathToContext(ctx, path, false);
            }
    ...
    blurredCopy(source, blurRadiusPx) {
        const out = new Canvas(source.width, source.height);
    ...
    effectLengthToPx(emu) {
        return emuToPixels(emu) * Math.min(this.scaleX, this.scaleY);
    }
    /**
 ...
 */
export function createEffectRenderer(scaleX, scaleY, logger) {
