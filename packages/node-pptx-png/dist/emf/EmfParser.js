 ...
 */
export function isEmf(buffer) {
    return (buffer.length >= 44 &&
        buffer.readUInt32LE(0) === EMR.HEADER &&
        buffer.readUInt32LE(40) === EMF_SIGNATURE);
}
/**
 * Parses EMF binaries into the shared metafile record model.
 */
export class EmfParser {
    logger;
    constructor(config = {}) {
        this.logger = config.logger ?? createLogger('warn', 'EmfParser');
    }
    /**
    ...
    parse(buffer) {
        ...
        if (iType !== EMR.HEADER || view.getUint32(40, true) !== EMF_SIGNATURE) {
    ...
    parseRecord(view, off, type, size, result) {
        ...
        switch (type) {
            ...
            case EMR.EXTTEXTOUTW:
                records.push(this.parseExtTextOut(view, off, size, type === EMR.EXTTEXTOUTW));
                break;
            case EMR.BITBLT:
...
//# sourceMappingURL=EmfParser.js.map